...
{
	int offset_ent, offset_dest, len_rel = *length_rel;
	char* entity;
	static char delim[4] = " \n\r\n";
	...
	int i, j, k, n, offset_rel;											
	for(i = 0; i < NAME.n_pointed; i++) {														//controllo tutte le relazioni che l'entità riceve
		
		offset_rel = check_rel(NAME.pointed_rel[i].id_rel, *relations, len_rel);						//qui mi salvo la relazione in esame (è uno strumento che ci servirà più tardi)
		...
		for(j = 0; j < NAME.pointed_rel[i].n_pnt ; j++){
			...
			if (REL.n_ent == 1 && REL.n_max == 1) {										//se la relazione aveva solo questa entità puntata da quella da cancellare allora devo togliere tutto
				...
				if (temp1 == NULL) {
					puts("ho cancellato tutte le relazioni");
					(*relations) = NULL;
					relations = NULL;
				}
				else {
	...
	if(length_names-1 != offset_rel) NAME = (*names)[length_names-1];
...
int add_rel(char* line_input, rel_type** relations, ent_type** names, int length_names, int length_rel)
{
	...
	static char s[4] = " \n\r\n";
	strtok(line_input, s);
    char* tid_orig = strtok(NULL, s);
    char* tid_dest = strtok(NULL, s);
    char* tid_rel = strtok(NULL, s);
	rel_type* temp1;
	rel* temp2;
    ...
    /*for(j = 0; j < 3; j++){
		...
		if(j == 0){
				tid_orig = realloc(tid_orig, (i+1)*sizeof(char));
                tid_orig[i] = '\0';
                }
		else { 
			if (j == 1){
				tid_dest = realloc(tid_dest, (i+1)*sizeof(char));
				tid_dest[i] = '\0';
				}
			else {
				tid_rel = realloc(tid_rel, (i+1)*sizeof(char));
				tid_rel[i] = '\0';
				}
			}
        b = b + 3;
...
int del_rel(char* line_input, rel_type** relations, ent_type** names, int length_names, int length_rel)
{
    int j, i, k;
	static char delim[4] = " \n\r\n";
	strtok(line_input, delim);
    char* tid_orig = strtok(NULL, delim);
    char* tid_dest = strtok(NULL, delim);
    char* tid_rel = strtok(NULL, delim);
						...
						if( strcmp(DEST.pointed_rel[i].ent_tx[j], tid_orig) == 0){			//se la relazione esiste procedo a rimuoverla
							...
							if(REL.n_ent == 1 && REL.n_max == 1){												//se questa era l'unica entità ricevente, rimuovo completamente la relazione
								if(length_rel == 1){
									...
									if (temp1 == NULL) {
										puts("Ho eliminato tutte le relazioni(delrel)");
										(*relations) = NULL;
										relations = NULL;
									}
									else{
